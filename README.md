...
entre 1.ª época, 2.ª época e época especial**.
...
| Épocas | `2.ª Época`, `Recurso`, `1E`, `Época Especial`; e blocos: depois de `Teste 2`, a `Nota Final 2` é da 2.ª época |
...
Duas regras que evitam os enganos mais comuns:

- **`Ex 2` não é a 2.ª época.** Um número no fim do cabeçalho só indica época se
  o resto do cabeçalho for de nota (`Nota Final 2`, `Teste 2`), nunca num
  exercício.
...
Entre as épocas de cada aluno em cada UC:
...
A nota mínima de aprovação é 9,5 e muda-se nos ajustes.
...
| Folha | O que traz |
|---|---|
| **Resumo** | Um aluno por linha, uma coluna por UC, média e nº de aprovações; gráfico da distribuição por escalão |
| **Uma por UC** | As três épocas lado a lado, melhor nota, época da melhor, estado, origem, e os componentes (agrupados — dá para recolher) |
...
de UC actualiza o resumo. A nota mínima está numa célula editável do Resumo.
...
colunas e épocas (incluindo os casos que enganam), a junção de alunos, a escolha
da melhor nota, os conflitos entre versões, o Excel gerado (valores, fórmulas e
o conjunto de funções seguras) e o percurso completo pela API.
